...
- (void)layoutPageViews
{
    ...
    for (UIScrollView *pageVerScrollView in _pageViewArray) {
        ...
        if (prePageView) {
            ...
            if (pageVerScrollView == _pageViewArray[_pageViewArray.count-1]) {
                [_horScrollView addConstraint:[NSLayoutConstraint constraintWithItem:pageVerScrollView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:_horScrollView attribute:NSLayoutAttributeRight multiplier:1 constant:0]];
            }
        }else {
        ...
        [_horScrollView addConstraint:[NSLayoutConstraint constraintWithItem:pageVerScrollView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:_horScrollView attribute:NSLayoutAttributeBottom multiplier:1 constant:0]];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (hitView == _headerContentView) {
        for (UIView *subview in [hitView.subviews reverseObjectEnumerator]) {
            if (subview == weakSelf.headerView
